...
class PostManager extends Manager
{
    ...
    public function getList()
     {
        $db = $this->dbConnect(); 
        $q = $db->query('SELECT id, title, content, DATE_FORMAT(date, \'%d/%m/%Y à %Hh%imin%ss\') FROM chapter ORDER BY date DESC LIMIT 0, 5');     
    ...
    {
        $db = $this->dbConnect(); 
        $q = $db->prepare('SELECT author, content, DATE_FORMAT(date, \'%d/%m/%Y à %Hh%imin%ss\') FROM chapter WHERE :id');
        $q->bindValue('id', $id);
        $q->execute();
        //$getPost = $q->fetch();
